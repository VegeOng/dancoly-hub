...
async function syncOrders(request: Request) {
  ...
  for (let i = 0; i < allOrderIds.length; i += 50) {
    ...
    // ── Batch upsert all orders in one call ──────────────────────────────
    ...
    if (orderErr) { console.error('Order batch upsert failed:', orderErr); continue }
    upsertedOrders += orderRows.length

    // ── Batch delete + re-insert items ───────────────────────────────────
    const batchIds = orders.map(o => o.id)
    await supabase.from('tiktok_order_items').delete().in('tiktok_order_id', batchIds)
    ...
    if (allItems.length > 0) {
      ...
      if (!itemErr) upsertedItems += allItems.length
    }
  }
